...
namespace Zaphat.Core
{
    public class Texture : GPUResource
    {
        ...
        {
            if (unit < 0)
                throw new ArgumentException("Texture unit to activate can only be 0 or larger");
            if (unit >= GPUCapabilities.MaxTextureUnits)
                throw new Exception(string.Format("The GPU does not support more than {0} texture units!", GPUCapabilities.MaxTextureUnits));
